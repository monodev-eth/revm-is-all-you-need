...
//different slots with eth_getStorageAt call to access each slot values, and see if they are storing the implementation address.
//So, if the returned value isn’t ZERO ADDRESS, we know that the contract bytecode is behind a proxy pattern, and use that address instead.
